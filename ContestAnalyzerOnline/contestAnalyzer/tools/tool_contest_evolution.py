...
class ToolContestEvolution(ToolBase):
    ...
    def apply_to_all(self, contest):
        ...
        if contest.contest == "cqww":
            ...
            # Count QSOs
            contest.log["qsos_counter_band"] = contest.log[contest.log["isdupe"] == 0].groupby(
                ["band", "call"]).cumcount() + 1
            ...
            #            # Count points
            contest.log["points"] = np.where(
                (contest.log["continent"] == contest.log["mycontinent"]) & (contest.log["isdupe"] == 0) & (
                            pd.isnull(contest.log["continent"]) == False), 1,
                np.where((contest.log["continent"] != contest.log["mycontinent"]) & (contest.log["isdupe"] == 0) & (
                            pd.isnull(contest.log["continent"]) == False), 3, 0))

        # CQ WPX
        elif contest.contest == "cqwpx":

            pass
            # TODO - Develop get_prefix(call) function
            # TODO - Use this information to compute score of the contest. This implies to read the log differently (otherwise number = zone)
            # TODO - Update html page so that the right info is shown in the summary.
